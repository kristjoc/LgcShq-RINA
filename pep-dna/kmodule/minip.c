...
#define MINIP_SIZE (sizeof(struct minip_hdr)+sizeof(struct iphdr))
...
{
	if (!entry) {
		pep_debug("Entry to be destroyed not found in rtx queue");
                return -1;
        }
...
int rtxq_rtx(struct rtxq * q, u32 seq)
{
        if (!q)
                return -1;
        ...
        if (rtxqueue_entries_rtx(q->queue, seq) < 0) {
		/* rcu_read_unlock(); */
		/* rtxq_destroy(q); */
		return -1;
        }
...
int rtxq_ack(struct rtxq *q, u32 seq_num)
{
	int rc = 0;
        ...
        spin_lock_bh(&q->lock);
        rc = rtxqueue_entries_ack(q->queue, seq_num);
        /* rtimer_restart(&q->parent->timers.rtx, tr); */
 ...
 * -------------------------------------------------------------------------- */
int pepdna_minip_conn_delete(u32 conn_id, u8 *to_mac)
{
	/* FIXME */
 ...
 * -------------------------------------------------------------------------- */
int pepdna_minip_conn_finished(u32 conn_id, u8 *to_mac)
{
	/* FIXME */
	struct net_device *dev = dev_get_by_name(&init_net, ifname);
	struct minip_hdr *hdr;
...
int pepdna_minip_conn_response(u32 conn_id, u8 *to_mac)
{
	/* FIXME */
	struct net_device *dev = dev_get_by_name(&init_net, ifname);
...
static int pepdna_minip_conn_ack(u32 conn_id, u32 ack, u8 *to_mac)
{
	/* FIXME */
	struct net_device *dev = dev_get_by_name(&init_net, ifname);
	static uint16_t proto = ETH_P_MINIP;
				 ...
				 unsigned char *buf, size_t len)
{
	...
	skb->pkt_type = PACKET_OUTGOING;
		...
		pep_debug("Failed to copy skb with sn %u to rtx queue",
			  con->seq_to_send);
		kfree_skb(cskb);
		goto out;
        }
...
void pepdna_minip_handshake(struct work_struct *work)
{
	...
	rc = pepdna_minip_conn_request(con->tuple.saddr, con->tuple.source,
				       ...
				       con->hash_conn_id, con->server->to_mac);
	if (rc < 0) {
...
int pepdna_minip_skb_callback(struct sk_buff *skb)
{
	...
	case MINIP_CONN_RESPONSE:
		...
		if (!con) {
			pep_err("Conn. instance %u not found", hash_id);
			kfree_skb(skb);
			return NET_RX_DROP;
		}
		...
		if (unlikely(rc <= 0)) {
			if (unlikely(rc == -EAGAIN)) {
				...
				pep_debug("Sending MINIP_CONN_DELETE with cid %u", con->hash_conn_id);
                                if (pepdna_minip_conn_delete(con->hash_conn_id,
                                                             con->server->to_mac) < 0) {
		...
		if (!con) {
			...
			return NET_RX_DROP;
                ...
                pepdna_minip_conn_finished(con->hash_conn_id,
                                           con->server->to_mac);
		pep_debug("Sent MINIP_CONN_FINISHED with cid %u", hash_id);
...
static int pepdna_con_i2minip_fwd(struct pepdna_con *con)
{
	...
	pep_debug("read %d/%zu bytes from TCP sock", read, how_much);
	if (likely(read > 0)) {
...
static int pepdna_con_minip2i_fwd(struct pepdna_con *con, struct sk_buff *skb)
{
	...
	pepdna_minip_conn_ack(con->hash_conn_id, con->ack_to_send, con->server->to_mac);
	...
	while (lconnected(con)) {
		if ((rc = pepdna_con_i2minip_fwd(con)) <= 0) {
			if (rc == -EAGAIN) { // FIXME Handle -EAGAIN flood
				pep_debug(
					  "No TCP data available or Zero MINIP window "
					  "Try again later...");
				break;
			}
